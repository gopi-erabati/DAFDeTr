...
class RandomScaleImageMultiViewImage(object):
    ...
    def __init__(self, scales=[0.5, 1.0, 1.5], img_shape=[448, 800]):
        self.scales = scales
    ...
    def __call__(self, results):
        ...
        rand_scale = self.scales[0]
        img_shape = results['img_shape']
        y_size = self.img_shape[0]
        x_size = self.img_shape[1]
        results['img'] = [mmcv.imresize(img, (x_size, y_size), return_scale=False) for img in results['img']]
        scale_factor = np.eye(4)
        scale_factor[0, 0] *= self.img_shape[0] / img_shape[0]
        scale_factor[1, 1] *= self.img_shape[1] / img_shape[1]
        lidar2img = [scale_factor @ l2i for l2i in results['lidar2img']]
        results['lidar2img'] = lidar2img
        results['img_shape'] = [img.shape for img in results['img']]
        # results['gt_bboxes_3d'].tensor[:, :6] *= rand_scale
